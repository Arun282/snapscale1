...
app.get("/api/health",(req,res)=>res.json({ok:true,name:"SnapScale Server",version:"4.0.0",advancedEditor:true,processing:"sharp",online:true,ai:false}));
...
app.post("/api/edit",upload.single("image"),async(req,res)=>{
 try{
  ...
  const brightness=clamp(n(q.brightness,1),.1,3),exposure=clamp(n(q.exposure,0),-100,100);
  const saturation=clamp(n(q.saturation,1),0,3),contrast=clamp(n(q.contrast,1),.1,3);
  ...
  const highlights=clamp(n(q.highlights,0),-100,100),shadows=clamp(n(q.shadows,0),-100,100);
  const whites=clamp(n(q.whites,0),-100,100),blacks=clamp(n(q.blacks,0),-100,100);
  img=img.linear(1+(highlights+whites)*.0015,(shadows-blacks)*.55);

  const fade=clamp(n(q.fade,0),0,100);
  if(fade)img=img.linear(1-fade/500,fade*.5).modulate({saturation:1-fade/250});
  ...
  if(sharpness)img=img.sharpen({sigma:clamp(.6+sharpness*.35,.6,3)});
  if(clarity)img=img.sharpen({sigma:1,flat:1,jagged:2});
...
app.get("/api/admin/stats",admin,(req,res)=>res.json({
 server:"Healthy",editor:"Online",auth:"JWT",storage:"Local processing",ai:"Disabled by design",
